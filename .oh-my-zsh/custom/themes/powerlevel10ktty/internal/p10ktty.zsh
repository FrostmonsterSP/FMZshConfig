if [[ $__p9k_sourced != 13 ]]; then
  >&2 print -P ""
  >&2 print -P "[%F{1}ERROR%f]: Corrupted powerlevel10ktty installation."
  >&2 print -P ""
  if (( ${+functions[antigen]} )); then
    ...
    >&2 print -P "reinstalling powerlevel10ktty from scratch."
  fi
  >&2 print -P ""
  return 1
fi
...
if ! autoload -Uz is-at-least || ! is-at-least 5.1; then
  () {
    >&2 echo -E "You are using ZSH version $ZSH_VERSION. The minimum required version for Powerlevel10ktty is 5.1."
...
function p9k_prompt_segment() { p10ktty segment "$@" }
...
# the observable behavior of Powerlevel10ktty.
...
_p9k_prompt_wifi_async() {
  ...
  local last_tx_rate ssid link_auth rssi noise bars on out line v state iface
  {
    if [[ -x $airport ]]; then
      ...
      # Content example (https://github.com/romkatv/powerlevel10ktty/pull/973#issuecomment-680251804):
      #
      ...
      # Output example (https://github.com/romkatv/powerlevel10ktty/pull/973#issuecomment-680251804):
      #
...
# YSU_HARDCORE=1. See https://github.com/romkatv/powerlevel10ktty/issues/427.
_p9k_preexec1() {
...
_p9k_dump_instant_prompt() {
  ...
  local prompt_dir=${root_dir}/p10ktty-$user
  local root_file=$root_dir/p10ktty-instant-prompt-$user.zsh
  ...
  function p10ktty() {
    ...
    for opt in "${@:$OPTIND}"; do
      ...
      if (( $+functions[p10ktty-on-init] )); then
        >&$fd print -r -- '
  p10ktty-on-init() { '$functions[p10ktty-on-init]' }'
      fi
      if (( $+functions[p10ktty-on-pre-prompt] )); then
        >&$fd print -r -- '
  p10ktty-on-pre-prompt() { '$functions[p10ktty-on-pre-prompt]' }'
      fi
      if (( $+functions[p10ktty-on-post-prompt] )); then
        >&$fd print -r -- '
  p10ktty-on-post-prompt() { '$functions[p10ktty-on-post-prompt]' }'
      fi
      if (( $+functions[p10ktty-on-post-widget] )); then
        >&$fd print -r -- '
  p10ktty-on-post-widget() { '$functions[p10ktty-on-post-widget]' }'
      fi
      if (( $+functions[p10ktty-on-init] )); then
        >&$fd print -r -- '
  p10ktty-on-init'
  ...
  _p9k__display_v[$idx]=hide"
      ...
      if (( $+functions[p10ktty-on-pre-prompt] )); then
        >&$fd print -r -- '
  p10ktty-on-pre-prompt'
      fi
      if (( $+functions[p10ktty-on-init] )); then
        >&$fd print -r -- '
  unfunction p10ktty-on-init'
      fi
      if (( $+functions[p10ktty-on-pre-prompt] )); then
        >&$fd print -r -- '
  unfunction p10ktty-on-pre-prompt'
      fi
      if (( $+functions[p10ktty-on-post-prompt] )); then
        >&$fd print -r -- '
  unfunction p10ktty-on-post-prompt'
      fi
      if (( $+functions[p10ktty-on-post-widget] )); then
        >&$fd print -r -- '
  unfunction p10ktty-on-post-widget'
      fi
      >&$fd print -r -- '
  () {
'$functions[_p9k_init_toolbox]'
  }
  trap "unset -m _p9k__\*; unfunction p10ktty" EXIT
  ...
  typeset -g __p9k_instant_prompt_output=$tmpdir/p10ktty-instant-prompt-output-${(%):-%n}-$$
  ...
  typeset -g __p9k_instant_prompt_dump_file=${XDG_CACHE_HOME:-~/.cache}/p10ktty-dump-${(%):-%n}.zsh
  ...
  function _p9k_instant_prompt_cleanup() {
    ...
    zf_rm -f -- $__p9k_instant_prompt_output $__p9k_instant_prompt_dump_file{,.zwc} $root_dir/p10ktty-instant-prompt-$user.zsh{,.zwc} $root_dir/p10ktty-$user/prompt-*(N) 2>/dev/null
  }
...
function _p9k_delete_instant_prompt() {
  local user=${(%):-%n}
  local root_dir=${__p9k_dump_file:h}
  zf_rm -f -- $root_dir/p10ktty-instant-prompt-$user.zsh{,.zwc} ${root_dir}/p10ktty-$user/prompt-*(N) 2>/dev/null
}
...
function _p9k_clear_instant_prompt() {
  ...
  if [[ -s $__p9k_instant_prompt_output ]]; then
    {
      ...
      if [[ -n $unexpected ]]; then
        ...
        if [[ $unexpected != ($omz1|)$omz2*($omz3|$omz4)[^$'\n']#($'\n'|) ]]; then
          ...
          echo -E - ""
          echo -E - "${(%):-When using Powerlevel10ktty with instant prompt, console output during zsh}"
          ...
          echo -E - ""
          echo -E - "${(%):-  - Suppress this warning either by running %Bp10ktty configure%b or by manually}"
          ...
          echo -E - ""
          echo -E - "${(%):-  - Disable instant prompt either by running %Bp10ktty configure%b or by manually}"
          echo -E - "${(%):-    defining the following parameter:}"
          echo -E - ""
          ...
          if (( _p9k_term_has_href )); then
            echo    - "${(%):-\e]8;;https://github.com/romkatv/powerlevel10ktty/blob/master/README.md#instant-prompt\ahttps://github.com/romkatv/powerlevel10ktty/blob/master/README.md#instant-prompt\e]8;;\a}"
          else
            echo    - "${(%):-https://github.com/romkatv/powerlevel10ktty/blob/master/README.md#instant-prompt}"
          fi
          echo -E - ""
  ...
  if [[ $_POWERLEVEL9K_DISABLE_INSTANT_PROMPT == 0 && $__p9k_instant_prompt_active == 2 ]]; then
    >&2 echo -E - ""
    >&2 echo -E - "${(%):-[%1FERROR%f]: When using Powerlevel10ktty with instant prompt, %Bprompt_cr%b must be unset.}"
    >&2 echo -E - ""
    >&2 echo -E - "${(%):-You can:}"
    >&2 echo -E - ""
    >&2 echo -E - "${(%):-  - %BRecommended%b: call %Bp10ktty finalize%b at the end of %B$__p9k_zshrc_u%b.}"
    >&2 echo -E - "${(%):-    You can do this by running the following command:}"
    >&2 echo -E - ""
    >&2 echo -E - "${(%):-      %2Fecho%f %3F'(( ! \${+functions[p10ktty]\} )) || p10ktty finalize'%f >>! $__p9k_zshrc_u}"
    >&2 echo -E - ""
    ...
    >&2 echo -E - "${(%):-    * Zsh will start %Bquickly%b and %Bwithout%b prompt flickering.}"
    >&2 echo -E - ""
    >&2 echo -E - "${(%):-  - Disable instant prompt either by running %Bp10ktty configure%b or by manually}"
...
function _p9k_on_expand() {
  ...
  if (( ! _p9k__expanded )); then
    ...
    if (( _POWERLEVEL9K_PROMPT_ADD_NEWLINE )); then
      ...
      elif [[ -z $_p9k_transient_prompt && $+functions[p10ktty-on-post-prompt] == 0 ]]; then
        _p9k__empty_line_i=3
  ...
  if (( ! _p9k__expanded )); then
    ...
    (( _p9k__fully_initialized || ! $+functions[p10ktty-on-init] )) || p10ktty-on-init
    ...
    (( $+functions[p10ktty-on-pre-prompt] )) && p10ktty-on-pre-prompt
...
function _p9k_on_widget_zle-line-finish() {
  ...
  (( $+functions[p10ktty-on-post-prompt] )) && p10ktty-on-post-prompt
...
function _p9k_widget_hook() {
  ...
  if (( ${+functions[p10ktty-on-post-widget]} || ${#_p9k_show_on_command} )); then
  ...
  (( $+functions[p10ktty-on-post-widget] )) && p10ktty-on-post-widget "${@:2}"
...
function _p9k_widget_zle-line-pre-redraw-impl() {
  ...
  ! (( ${+functions[p10ktty-on-post-widget]} || ${#_p9k_show_on_command} || _p9k__restore_prompt_fd || _p9k__redraw_fd )) &&
      [[ ${KEYMAP:-} != vicmd ]] &&
      return
...
function _p9k_wrap_widgets() {
  ...
  if is-at-least 5.3; then
    ...
    local keymap tmp=$tmpdir/p10ktty.bindings.$sysparams[pid]
    {
...
_p9k_must_init() {
  ...
  _p9k__param_pat+=$'$__p9k_root_dir\1$functions[p10ktty-on-init]\1$functions[p10ktty-on-pre-prompt]\1'
  _p9k__param_pat+=$'$functions[p10ktty-on-post-widget]\1$functions[p10ktty-on-post-prompt]\1'
...
_p9k_init() {
  ...
  if (( $+__p9k_instant_prompt_erased )); then
    ...
    {
      >&2 echo -E - ""
      >&2 echo -E - "${(%):-[%1FERROR%f]: When using instant prompt, Powerlevel10ktty must be loaded before the first prompt.}"
      >&2 echo -E - ""
      >&2 echo -E - "${(%):-You can:}"
      >&2 echo -E - ""
      >&2 echo -E - "${(%):-  - %BRecommended%b: Change the way Powerlevel10ktty is loaded from %B$__p9k_zshrc_u%b.}"
      if (( _p9k_term_has_href )); then
        >&2 echo    - "${(%):-    See \e]8;;https://github.com/romkatv/powerlevel10ktty/blob/master/README.md#installation\ahttps://github.com/romkatv/powerlevel10ktty/blob/master/README.md#installation\e]8;;\a.}"
      else
        >&2 echo    - "${(%):-    See https://github.com/romkatv/powerlevel10ktty/blob/master/README.md#installation.}"
      fi
      if (( $+zsh_defer_options )); then
        >&2 echo -E - ""
        >&2 echo -E - "${(%):-    NOTE: Do not use %1Fzsh-defer%f to load %Upowerlevel10ktty.zsh-theme%u.}"
      elif (( $+functions[zinit] )); then
        >&2 echo -E - ""
        >&2 echo -E - "${(%):-    NOTE: If using %2Fzinit%f to load %3F'romkatv/powerlevel10ktty'%f, %Bdo not apply%b %1Fice wait%f.}"
      elif (( $+functions[zplugin] )); then
        >&2 echo -E - ""
        >&2 echo -E - "${(%):-    NOTE: If using %2Fzplugin%f to load %3F'romkatv/powerlevel10ktty'%f, %Bdo not apply%b %1Fice wait%f.}"
      fi
      >&2 echo -E - ""
      >&2 echo -E - "${(%):-    * You %Bwill not%b see this error message again.}"
...
typeset -gr __p9k_p10ktty_usage="Usage: %2Fp10ktty%f %Bcommand%b [options]
...
Print help for a specific command:
  ...
  %2Fp10ktty%f %Bhelp%b command"

typeset -gr __p9k_p10ktty_segment_usage="Usage: %2Fp10ktty%f %Bsegment%b [-h] [{+|-}re] [-s state] [-b bg] [-f fg] [-i icon] [-c cond] [-t text]
...
- If you have permissions to delete the file, state is DELETABLE. If not, it's PROTECTED.
  ...
  function prompt_core() {
    ...
    p10ktty segment -s \$state -i '⭐' -f blue -t \${size[1]}b
  }
...
typeset -gr __p9k_p10ktty_configure_usage="Usage: %2Fp10ktty%f %Bconfigure%b

Run interactive configuration wizard."

typeset -gr __p9k_p10ktty_reload_usage="Usage: %2Fp10ktty%f %Breload%b

Reload configuration."

typeset -gr __p9k_p10ktty_finalize_usage="Usage: %2Fp10ktty%f %Bfinalize%b

Perform the final stage of initialization. Must be called at the very end of zshrc."

typeset -gr __p9k_p10ktty_display_usage="Usage: %2Fp10ktty%f %Bdisplay%b part-pattern=state-list...

  Show, hide or toggle prompt parts. If called from zle, the current
  prompt is refreshed.

Usage: %2Fp10ktty%f %Bdisplay%b -a [part-pattern]...

  Populate array \`reply\` with states of prompt parts matching the patterns.
  If no patterns are supplied, assume \`*\`.

Usage: %2Fp10ktty%f %Bdisplay%b -r
...
Example: Bind Ctrl+P to toggle right prompt.
  ...
  function toggle-right-prompt() { p10ktty display '*/right'=hide,show; }
...
function p10ktty() {
  [[ $# != 1 || $1 != finalize ]] || { p10ktty-instant-prompt-finalize; return 0 }

  eval "$__p9k_intro_no_reply"

  if (( !ARGC )); then
    print -rP -- $__p9k_p10ktty_usage >&2
    return 1
  fi
  ...
  case $1 in
    segment)
      ...
      while getopts ':s:b:f:i:c:t:reh' opt; do
        case $opt in
          ...
          +r) ref=0;;
          +e) expand=0;;
          h) print -rP -- $__p9k_p10ktty_segment_usage; return 0;;
          ?) print -rP -- $__p9k_p10ktty_segment_usage >&2; return 1;;
        esac
      done
      if (( OPTIND <= ARGC )); then
        print -rP -- $__p9k_p10ktty_segment_usage >&2
        return 1
      fi
      if [[ -z $_p9k__prompt_side ]]; then
        print -rP -- "%1F[ERROR]%f %Bp10ktty segment%b: can be called only during prompt rendering." >&2
        if (( !ARGC )); then
          print -rP -- ""
          print -rP -- "For help, type:" >&2
          print -rP -- ""
          print -rP -- "  %2Fp10ktty%f %Bhelp%b %Bsegment%b" >&2
        fi
        return 1
      fi
      (( ref )) || icon=$'\1'$icon
    ...
    display)
      if (( ARGC == 1 )); then
        print -rP -- $__p9k_p10ktty_display_usage >&2
        return 1
      fi
      shift
      local -i k dump
      ...
      while getopts ':har' opt; do
        case $opt in
          ...
          ;;
          a) dump=1;;
          h) print -rP -- $__p9k_p10ktty_display_usage; return 0;;
          ?) print -rP -- $__p9k_p10ktty_display_usage >&2; return 1;;
        esac
      done
      if (( dump )); then
        reply=()
    ...
    configure)
      if (( ARGC > 1 )); then
        print -rP -- $__p9k_p10ktty_configure_usage >&2
        return 1
      fi
      local REPLY
      local -a reply
    ...
    reload)
      if (( ARGC > 1 )); then
        print -rP -- $__p9k_p10ktty_reload_usage >&2
        return 1
      fi
      ...
      ;;
    help)
      local var=__p9k_p10ktty_$2_usage
      if (( $+parameters[$var] )); then
        print -rP -- ${(P)var}
        return 0
      elif (( ARGC == 1 )); then
        print -rP -- $__p9k_p10ktty_usage
        return 0
      else
        print -rP -- $__p9k_p10ktty_usage >&2
        return 1
      ...
      ;;
    finalize)
      print -rP -- $__p9k_p10ktty_finalize_usage >&2
      return 1
      ;;
...
# Hook for zplugin.
powerlevel10ktty_plugin_unload() { prompt_powerlevel9k_teardown; }

function p10ktty-instant-prompt-finalize() {
