...
function quit() {
  ...
  print
  if (( force )); then
    flowing Powerlevel10ktty configuration wizard has been aborted. To run it again, type:
    print -P ""
    print -P "  %2Fp10ktty%f %Bconfigure%b"
    print -P ""
  else
    flowing                                                                        \
      Powerlevel10ktty configuration wizard has been aborted. It will run again       \
      next time unless you define at least one Powerlevel10ktty configuration option. \
      To define an option that does nothing except for disabling Powerlevel10ktty     \
    ...
    print -P ""
    flowing To run Powerlevel10ktty configuration wizard right now, type:
    print -P ""
    print -P "  %2Fp10ktty%f %Bconfigure%b"
    print -P ""
  fi
    ...
    while true; do
      while true; do
        ...
        if (( wizard_columns < __p9k_wizard_columns )); then
          ...
          print
          flowing Make terminal window %Bwider%b or press %BCtrl-C%b to abort Powerlevel10ktty configuration wizard.
        elif (( wizard_lines < __p9k_wizard_lines )); then
          clear
          flowing -c %1FNot enough vertical space.%f
          print
          flowing Make terminal window %Btaller%b or press %BCtrl-C%b to abort Powerlevel10ktty configuration wizard.
        else
          break
        fi
...
function print_greeting() {
  ...
  if (( in_z4h_wizard )); then
    flowing -c %3FZsh for Humans%f uses %4FPowerlevel10ktty%f to print command        \
               line prompt. This wizard will ask you a few questions and configure \
               prompt for you.
  elif (( force )); then
    flowing -c This is %4FPowerlevel10ktty configuration wizard%f. \
               It will ask you a few questions and configure your prompt.
  else
    flowing -c This is %4FPowerlevel10ktty configuration wizard%f.   \
               You are seeing it because you haven\'t defined any \
               Powerlevel10ktty configuration options. It will ask   \
...
function ask_remove_font() {
  ...
  if (( protected )); then
    if (( $#fonts == 1 )); then
      add_widget 0 flowing Please %Bdelete%b this file and run '%2Fp10ktty%f %Bconfigure%b.'
    else
      add_widget 0 flowing Please %Bdelete%b these files and run '%2Fp10ktty%f %Bconfigure%b.'
    fi
    add_widget 0 print
    restore_screen
    local pass render
...
function print_instant_prompt_link() {
  local link='https://github.com/romkatv/powerlevel10ktty/blob/master/README.md#instant-prompt'
...
function ask_config_overwrite() {
  ...
  add_widget 0 flowing -c Powerlevel10ktty config file already exists.
...
function generate_config() {
  local base && base="$(<$__p9k_root_dir/config/p10ktty-${style//_/-}.zsh)" || return
  local lines=("${(@f)base}")
  ...
  local header=${(%):-"# Generated by Powerlevel10ktty configuration wizard on %D{%Y-%m-%d at %H:%M %Z}."}$'\n'
  header+="# Based on romkatv/powerlevel10ktty/config/p10ktty-${style//_/-}.zsh"
  ...
  header+=$line
  header+=$'.\n# Type `p10ktty configure` to generate another config.\n#'
...
function change_zshrc() {
...

  {
    ...
    if (( !zshrc_has_instant_prompt )); then
      >>$tmp print -r -- "# Enable Powerlevel10ktty instant prompt. Should stay close to the top of ${(%)__p9k_zshrc_u}.
# Initialization code that may require console input (password prompts, [y/n]
# confirmations, etc.) must go above this block; everything else may go below.
if [[ -r \"\${XDG_CACHE_HOME:-\$HOME/.cache}/p10ktty-instant-prompt-\${(%):-%n}.zsh\" ]]; then
  source \"\${XDG_CACHE_HOME:-\$HOME/.cache}/p10ktty-instant-prompt-\${(%):-%n}.zsh\"
fi" || return
    fi
    if [[ -n $zshrc_content ]]; then
    ...
    if (( !zshrc_has_cfg )); then
      >>$tmp print -r -- "
# To customize prompt, run \`p10ktty configure\` or edit ${(%)__p9k_cfg_path_u}.
...
function check_zshrc_integration() {
  ...
  local h0='${ZDOTDIR:-~}/.p10ktty.zsh'
  local h1='${ZDOTDIR:-$HOME}/.p10ktty.zsh'
  local h2='"${ZDOTDIR:-$HOME}/.p10ktty.zsh"'
  local h3='"${ZDOTDIR:-$HOME}"/.p10ktty.zsh'
  local h4='${ZDOTDIR}/.p10ktty.zsh'
  local h5='"${ZDOTDIR}/.p10ktty.zsh"'
  local h6='"${ZDOTDIR}"/.p10ktty.zsh'
  local h7='$ZDOTDIR/.p10ktty.zsh'
  local h8='"$ZDOTDIR/.p10ktty.zsh"'
  local h9='"$ZDOTDIR"/.p10ktty.zsh'
  ...
  local pre='${XDG_CACHE_HOME:-$HOME/.cache}/p10ktty-instant-prompt-${(%):-%n}.zsh'
...
() {
  ...
  print -P ""
  flowing                                                                          \
      Powerlevel10ktty configuration file "($__p9k_cfg_path_u)" was not sourced. This \
      ...
      there are no errors, try running Powerlevel10ktty configuration wizard:
  print -P ''
  print -P '  %2Fp10ktty%f %Bconfigure%b'
  print -P ''
  flowing                                                                              \
...
if (( !in_z4h_wizard )); then
  print -rP ""
  flowing +c File feature requests and bug reports at "$(href https://github.com/romkatv/powerlevel10ktty/issues)"
  print -rP ""
fi
