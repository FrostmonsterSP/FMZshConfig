...
function _p9k_can_configure() {
  ...
  function $0_error() {
    (( q )) || print -rP "%1F[ERROR]%f %Bp10ktty configure%b: $1" >&2
  }
  typeset -g __p9k_cfg_path_o=${POWERLEVEL9K_CONFIG_FILE:=${ZDOTDIR:-~}/.p10ktty.zsh}
  ...
  typeset -g __p9k_cfg_path_u=${${${(q)__p9k_cfg_path_o}/#(#b)${(q)HOME}(|\/*)/'~'$match[1]}//\%/%%}
  {
    ...
    for style in lean lean-8colors classic rainbow pure; do
      [[ -r $__p9k_root_dir/config/p10ktty-$style.zsh ]]                      || {
        $0_error "$__p9k_root_dir_u/config/p10ktty-$style.zsh is not readable"
        return 1
      }
    done
